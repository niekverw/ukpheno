...
#  StrDescription<-"asd"
#  HEScodes<-"Z825"
 # filesEpisode<-c("/data_work/databases/ukbiobanks/Phenotypes/test/ts_53_2_0/HESIN/Ca_D9hes.dta",
 #                 "/data_work/databases/ukbiobanks/Phenotypes/test/ts_53_2_0/HESIN/Ca_Dhes.dta", 
#                  "/data_work/databases/ukbiobanks/Phenotypes/test/ts_53_2_0/HESIN/Ca_DO.dta",
#                  "/data_work/databases/ukbiobanks/Phenotypes/test/ts_53_2_0/HESIN/Ca_DOp.dta")
 ...
 print(paste("merging HESIN",filesEpisode))
  ...
  VctDescriptions<-unname(c(VctDescriptions[1],VctDescriptions[VctDescriptions!="Identifier"]))
  ...
  if ( length(which(grepl( "_HXn" , names( dfMerged ) )))>1){
    ...
    HXd<-rowMins( as.matrix(dfMerged[ , grepl( "_HXd" , names( dfMerged ) ) ]) , na.rm=T)
    HXd[HXd==Inf]<-NA
    dfMerged$HXd<-HXd
  ...
  if (length(which(grepl( "_FUn" , names( dfMerged ) )))>1){
    ...
    FUd<-rowMins( as.matrix(dfMerged[ , grepl( "_FUd" , names( dfMerged ) ) ]) , na.rm=T)
    FUd[FUd==Inf]<-NA
    dfMerged$FUd<-FUd
  ...
  cnames<-colnames(dfMerged)[(length(dfMerged)-3):length(dfMerged)] 
  colnames(dfMerged)[(length(dfMerged)-3):length(dfMerged)]  <-paste(StrTrait,"_HES_",cnames,sep="")
...
#  StrTrait<-"HF"
  ...
  VctAllColumns<-c(StrTrait_HXd,StrTrait_HXn,StrTrait_FUd,StrTrait_FUn,StrTrait_UKBV_HXn,StrTrait_UKBV_HXdlv,StrTrait_UKBV_HXd,StrTrait_UKBV_FUd,StrTrait_UKBV_FUdlv,StrTrait_UKBV_FUn,StrTrait_TSAd)
