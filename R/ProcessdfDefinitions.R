...
ProcessDfDefinitions<-function(df,VctAllColumns=c("TS", "SR", "TS_RX", "RX", "LAB", "ICD10CODES", "ICD9CODES", "OPERCODES", "TS_AGE_DIAG_COLNAME", "READCODES", "n_20001_",    "n_20002_", "n_20003_", "n_20004_", "DEPENDENCY")){
 ...
 # df<- dfDefinitions  #  df<- dfDefinitions2
  ...
  # df = excel matrix. { 1) hij loopt een voor een over elke rij heen, 2) zoekt per dependency in die rij de bijpassende rijen voor elke dependency  erbij en plakt die naast elkaar (inclusief de dependencies van de dependencies). 4) dan delete hij de depenencies die hij ingevuld heeft. }  <— dit op repeat tot dat er geen dependencies meer zijn en alles is ingevuld. 
  repeat {
    for(i in 1:nrow(df)) {
      row <- df[i,]
